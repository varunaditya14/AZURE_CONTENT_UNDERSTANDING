"""
Pro-mode test file sets router — persistent named collections of test files.

Storage layout
--------------
  <backend>/data/test_file_sets.json          — JSON index (list of set records)
  <backend>/data/test_file_sets/<set_id>/     — per-set directory
      <file_id>_<original_filename>           — stored file bytes

Endpoints
---------
  GET    /pro/test-file-sets                       — list all saved sets
  POST   /pro/test-file-sets                       — create set, upload files
  PUT    /pro/test-file-sets/{id}                  — update name / add-remove files
  DELETE /pro/test-file-sets/{id}                  — delete set and its files
  GET    /pro/test-file-sets/{id}/files/{file_id}  — serve a stored file
"""
...
router = APIRouter(prefix="/pro/test-file-sets", tags=["pro"])
...
SETS_DIR = DATA_DIR / "test_file_sets"
INDEX_FILE = DATA_DIR / "test_file_sets.json"
...
async def list_test_file_sets() -> list[TestFileSetResponse]:
    _ensure_dirs()
...
async def create_test_file_set(
    name: str = Form(...),
    files: List[UploadFile] = File(default=[]),
) -> TestFileSetResponse:
    _ensure_dirs()
    set_id = f"tfs-{uuid.uuid4().hex[:10]}"
    set_dir = SETS_DIR / set_id
    set_dir.mkdir(parents=True, exist_ok=True)

    file_items: list[dict] = []
    for uf in files:
        file_id = f"tffile-{uuid.uuid4().hex[:10]}"
...
async def update_test_file_set(
    set_id: str,
    name: str = Form(...),
    ...
    if idx is None:
        raise HTTPException(
            ...
            detail="Test file set not found",
        )
...
async def delete_test_file_set(set_id: str) -> None:
    _ensure_dirs()
    sets = _load_index()
    idx = next((i for i, s in enumerate(sets) if s["id"] == set_id), None)
    if idx is None:
...
async def get_test_file(set_id: str, file_id: str) -> FileResponse:
    set_dir = SETS_DIR / set_id
    ...
    # Serve inline so the browser can display the file rather than prompting
    # a save-dialog (which happens when Content-Disposition is "attachment").
    return FileResponse(
        str(path),
